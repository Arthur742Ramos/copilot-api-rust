...
- Compared surfaces: `src/routes/messages/` (handler, preprocessing, request,
  non-streaming and both streaming translators), `src/routes/provider/messages/`,
  `src/routes/models.ts`, `src/lib/error.ts`, and
...
| `POST /v1/messages/count_tokens` | Supported | `src/routes/messages/count_tokens_handler.rs`; tokenizer and route tests | Uses Anthropic's counter when configured; otherwise uses the documented local estimate. |
...
| Provider-scoped Messages and count-token routes | Supported | `src/routes/provider/messages.rs`; `src/routes/provider/count_tokens.rs` | Anthropic, OpenAI-compatible, Responses, and configured model aliases are covered. |
...
| Anthropic error envelopes and status-derived types | **Fixed here** | `src/libs/error.rs` tests `upstream_json_error_envelope_is_forwarded_verbatim` and `complete_upstream_error_envelope_is_preserved`; `tests/provider_routing.rs::unknown_provider_returns_complete_anthropic_404`; `router_smoke.rs` | 400/401/403/404/413/429/529/5xx map to complete Anthropic error envelopes. Nested-only upstream errors gain the SDK-required top-level discriminator without changing their nested error, status, or allowed headers. |
