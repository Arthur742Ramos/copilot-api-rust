...
| Native Anthropic stream truncation/control-frame JSON | **Fixed here** | Native stream drivers in `api_flows.rs` and `provider/messages.rs`; `api_flows.rs` test `native_message_parse_fast_path_skips_token_frames` | A native stream must end in upstream `message_stop` or `error`; silent EOF is converted to an error, not synthetic `message_stop`. Public native forwarding JSON-decodes only usage/terminal control frames, while high-volume content-block frames remain zero-copy passthrough. |
