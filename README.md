...
| `GET` | `/admin/config` | Read the effective merged runtime config with all secrets redacted (`adminApiKey`/`anthropicApiKey` → `*Set` booleans, `apiKeys` → count, provider `apiKey` → `apiKeySet`) (admin auth). |
